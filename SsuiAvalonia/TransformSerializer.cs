...
namespace SolidShineUi
{
    ...
    /// <summary>
    /// A helper class to serialize and deserialize Avalonia <see cref="Transform"/> objects to and from strings.
    /// </summary>
    /// <remarks>
    ...
    /// and then fall back to serializing the Transform's Matrix value for any remaining types. <see cref="SerializeAsMatrix(ITransform)"/> can also be used to
    ...
    /// <para/>
    /// This is fully compatible with the transform serialization strings from the WPF version of this class, although
    /// Avalonia's <see cref="SkewTransform"/> and <see cref="ScaleTransform"/> do not have a Center property like WPF's do.
    /// </remarks>
    public static class TransformSerializer
    {
        ...
        {
            return string.Join(separator, "tk", GetDouble(st.AngleX), GetDouble(st.AngleY), "0", "0");
        }

        private static string SerializeScaleTransform(ScaleTransform st)
        {
            return string.Join(separator, "ts", GetDouble(st.ScaleX), GetDouble(st.ScaleY), "0", "0");
        }
        ...
        private static string SerializeTransformGroup(TransformGroup tg)
        {
            ...
            {
                return SerializeTransform(TransformHelper.IdentityTransform);
            }
            else
            {
        ...
        private static string SerializeTransformCollection(Transforms tg)
        {
        ...
        /// <see cref="Avalonia.Media.Immutable.ImmutableTransform"/> is also converted to a string by taking it's matrix value.
        /// </remarks>
        public static string SerializeTransform(ITransform transform)
        {
            ...
            {
                return string.Join(separator, "tm", transform.Value.ToString());
            }
        }
        ...
        /// </summary>
        /// <param name="transform">the transform to serialize</param>
        /// <remarks>See the remarks for <see cref="SerializeTransform(ITransform)"/> for more details.</remarks>
        public static string Serialize(this ITransform transform)
        {
        ...
        /// <param name="transform">the transform to serialize</param>
        /// <remarks>
        /// This will create a consistently formatted string, regardless of the type of transform, whereas <see cref="Serialize(ITransform)"/>
        /// will create special strings for serializing common Transform types.
        /// </remarks>
        public static string SerializeAsMatrix(this ITransform transform)
        {
            return string.Join(separator, "tm", transform.Value.ToString());
        }
        ...
        public static string Serialize(this Transforms tc)
        {
        ...
        /// using <see cref="Serialize(ITransform)"/> or <see cref="SerializeTransform(ITransform)"/>, then the returned object
            ...
            else
            {
                ...
                switch (vals[0].ToLowerInvariant())
                {
                    ...
                    case "tk":
                        ...
                        return new SkewTransform(ConvDouble(vals[1]), ConvDouble(vals[2]));
                    case "ts":
                        // scale transform
                        if (vals.Length < 5) throw new FormatException("Not valid transform serialization string");
                        return new ScaleTransform(ConvDouble(vals[1]), ConvDouble(vals[2]));
                    case "tt":
                        // translate transform
    ...
    /// <summary>
    /// A helper class for working with <see cref="Transform"/> objects in Avalonia.
    /// </summary>
    public static class TransformHelper
    {
        /// <summary>
        /// An identity transform, by creating an identity matrix. This transform does not actually change anything with the visual.
        /// </summary>
        public static Transform IdentityTransform { get; } = new MatrixTransform(Matrix.Identity);
    }
