...
namespace System.Windows.Shell
{
    ...
    using HANDLE_MESSAGE = Collections.Generic.KeyValuePair<WM, MessageHandler>;
    ...
    internal class WindowChromeWorker : DependencyObject
    {
        ...
        private static void _OnChromeWorkerChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (w != null && cw != null && cw._window == null)
            {
                cw._SetWindow(w);
            }
        }
        ...
        private void _FixupTemplateIssues()
        {
            if (_window != null && _chromeInfo != null)
            {
                ...
                {
                    if (IsFlagSet((int)_chromeInfo.NonClientFrameEdges, (int)NonClientFrameEdges.Top))
                    {
                        templateFixupMargin.Top -= SystemParameters.WindowResizeBorderThickness.Top;
                    }
                    if (IsFlagSet((int)_chromeInfo.NonClientFrameEdges, (int)NonClientFrameEdges.Left))
                    {
                        templateFixupMargin.Left -= SystemParameters.WindowResizeBorderThickness.Left;
                    }
                    if (IsFlagSet((int)_chromeInfo.NonClientFrameEdges, (int)NonClientFrameEdges.Bottom))
                    {
                        templateFixupMargin.Bottom -= SystemParameters.WindowResizeBorderThickness.Bottom;
                    }
                    if (IsFlagSet((int)_chromeInfo.NonClientFrameEdges, (int)NonClientFrameEdges.Right))
                    {
                ...
                if (IsPresentationFrameworkVersionLessThan4)
                {
                    DpiScale dpi = ((Visual)_window).GetDpi();
        ...
        private static void AddDependencyPropertyChangeListener(object component, DependencyProperty property, EventHandler listener)
        {
            if (component != null)
            {
        ...
        private static void RemoveDependencyPropertyChangeListener(object component, DependencyProperty property, EventHandler listener)
        {
            if (component != null)
            {
                DependencyPropertyDescriptor dependencyPropertyDescriptor = DependencyPropertyDescriptor.FromProperty(property, component.GetType());
        ...
        private static bool IsFlagSet(uint value, uint mask)
        {
            return (value & mask) != 0;
        }

        private static bool IsFlagSet(long value, long mask)
        {
            return (value & mask) != 0;
        }

        private static bool IsFlagSet(ulong value, ulong mask)
        {
            return (value & mask) != 0;
        }

        private static Thickness WindowResizeBorderThickness
        {
            [SecurityCritical]
            get
            {
                lock ()
                {
                    while (!_cacheValid[118])
                    {
                        ...
                        Size deviceSize = new Size(NativeMethods.GetSystemMetrics(SM.CXFRAME), Standard.NativeMethods.GetSystemMetrics(Standard.SM.CYFRAME));
                        Size size = DpiHelper.DeviceSizeToLogical(deviceSize, (double)DpiX / 96.0, (double)Dpi / 96.0);
